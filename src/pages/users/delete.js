...
import {DELETE_USER_BY_ID} from '../../routes/auth.js';
import {USER_DELETED_SUCCESS_MESSAGE} from '../../messages/message.js'

export default function DeleteUser({currentModalState, updateParentDeleteModalState, userId, deleteUser}) {
//   console.log({currentModalState, userId})
  ...
  const handleDelete = async() => {
    ...
    try {
      setError('');
      setSuccess('');
      const response = await fetch(DELETE_USER_BY_ID.replace(':id', userId), options);
      ...
      if (status){
        setSuccess(USER_DELETED_SUCCESS_MESSAGE);
        deleteUser(data.user); // this is setting up the state of the user list on index page
        handleClose();
      }
      else{
        // setRows([]);
